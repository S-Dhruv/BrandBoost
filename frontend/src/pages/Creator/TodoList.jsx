...
const TodoList = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#081A42]">
      <div className="relative z-10 w-full max-w-md p-8 bg-white rounded-3xl shadow-lg border border-[#328AB0]/20">
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-bold bg-gradient-to-r from-[#42A4E0] to-[#1D78A0] text-transparent bg-clip-text">
            Todo List
          </h1>
        </div>
        ...
        <form onSubmit={addTodo} className="mb-6 space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#42A4E0] text-white py-4 rounded-2xl hover:bg-[#1D78A0] focus:outline-none transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? <Loader2 className="h-5 w-5 animate-spin mx-auto" /> : 'Add Todo'}
          </button>
        </form>
        ...
        {loading && !todos.length ? (
          ...
          <ul className="space-y-3">
            {Array.isArray(todos) && todos.map((todo) => (
              <li
                key={todo._id}
                className="flex items-center justify-between p-4 bg-[#F9FAFB] rounded-2xl border-2 border-[#328AB0]/20 hover:border-[#42A4E0]/50 transition-all duration-300"
              >
